...
export const GET = api(undefined, async (ctx) => {
  ...
  const [rows] = await pool.query<Row[]>(
    `SELECT bs.id, bs.branch_id, u.name, u.email, bs.added_by, bs.created_at
...
export const POST = api(undefined, async (ctx) => {
  ...
  try {
    await withTransaction(async (conn) => {
      ...
      await conn.query(
        `INSERT INTO branch_staff (id, branch_id, user_id, added_by) VALUES (?, ?, ?, ?)`,
        [staffId, branchId, userId, auth.userId],
      );
    });
  } catch (err) {
